...
class ThermochimicaInputGenerator:
    ...
    def generate_input_files(self) -> None:
        ...
        for time_step, composition in time_steps.items():
            ...
            time_step_dir = os.path.join(self.output_dir, f"timestep_{time_step}")
    ...
    def _extract_elements_mole_percent(self, composition: Dict[str, Dict[str, float]]) -> Dict[str, float]:
        ...
        Returns:
            Dictionary mapping element symbols to mole percentages (normalized to sum to 1)
        """
        elements_dict = {}
        
        for element, data in composition.items():
            if "mole_percent" in data:
                elements_dict[element.lower()] = data["mole_percent"]
        
        # Normalize mole percentages to sum to 1
        total_mole_percent = sum(elements_dict.values())
        if total_mole_percent > 0:
            elements_dict = {e: v / 100.0 for e, v in elements_dict.items()}  # Convert from percent to fraction
    ...
    def _generate_input_file(self, file_path: str, elements: Dict[str, float], time_step: str) -> None:
        ...
        self.tc.header = f"Surrogate Vector Calculation for Time Step {time_step}"
    ...
    def run_calculations(self) -> None:
        """Run Thermochimica calculations for all generated input files"""
        for time_step in self.surrogate_data["surrogate_vector"]:
            time_step_dir = os.path.join(self.output_dir, f"timestep_{time_step}")
            input_file_name = f"{self.main_file_name}_t{time_step}.ti"
